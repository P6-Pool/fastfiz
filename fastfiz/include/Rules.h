...
enum Decision {
  ...
  DEC_RERACK, /**< Re-rack the balls and get a turn after opponent's bad break
                 */
  DEC_EIGHTBALL_RERACK_OPP_SHOOT, /**< If TT_EIGHTBALL_FOUL_ON_BREAK, the
                                     incoming player can decide to re-rack and
                                     have the opponent (original breaker)
                                     re-break. If they wimply want to re-rack
                                     and themselves shoot, choose DEC_RERACK*/
  DEC_CONCEDE,                    /**< concede the game, always allowed */
  DEC_NINEBALL_PUSH_OUT /**< In TT_NINEBALL_FIRST_SHOT, means declaring "Push
                           Out". In TT_NINEBALL_PUSH_OUT means player does not
                           accept position. */
};
...
enum TurnType {
  ...
  TT_EIGHTBALL_FOUL_ON_BREAK, /**< If there is a foul on the break. Needs a
                                 decision */
  TT_EIGHTBALL_8BALL_POCKETED_ON_BREAK, /**< If the eight-ball is pocketed on
                                           the break. Needs a decision */
  TT_NINEBALL_FIRST_SHOT, /**< First shot after a legal break in nine-ball */
  TT_NINEBALL_PUSH_OUT    /**< Incoming player's decision node after Push Out
                             (neesds to decide DEC_KEEP_SHOOTING or
 ...
 */
enum ShotResult {
  SR_OK, /**< The shot was executed, and the shooter won the game or may shoot
            again. */
  SR_OK_LOST_TURN, /**< The shot was executed, but the shooter has lost their
                      turn, and possibly the game. */
  SR_BAD_PARAMS,   /**< The shot information given is not applicable to the game
                      state. */
  SR_SHOT_IMPOSSIBLE, /**< The shot is not physically possible. For example, if
                         a ball is in the way of the cue stick. */
  SR_TIMEOUT /**< The agent took too much time computing the shot, and timed
                out. Shot was not executed. */
};
...
struct GameShot {
  ...
  /** Amount of time spent by the AI computing the shot. This value is ignored
   * during competition
   * however if the "fake time" option is set on the server, than the server
   * uses
...
public:
  ...
  explicit GameState(double timeleft = 0.0, double timeleft_opp = 0.0)
      : _tableState(), _turnType(TT_BREAK), _timeLeft(timeleft),
  ...
  virtual bool isTerminal() const {
    return _turnType == TT_RESERVED || _turnType == TT_WIN;
  };
  /**
   * Determines if the agent must provide a valid position for the cue ball.
   */
  virtual bool positionRequired() const {
    return _turnType == TT_BREAK || _turnType == TT_BALL_IN_HAND ||
           _turnType == TT_BEHIND_LINE;
  };
  /**
   * Determines if the agent is allowed to make a decision.
   */
  virtual bool decisionAllowed() const {
    return !shotRequired() && !isTerminal();
  };
  /**
  ...
  // Get Result of shot. In base class, this checks for timeout, calls
  // isAllowed, then tries
   ...
   * \param shotObj Pointer to an unallocated Shot* which will include a newly
   * allocated Shot object.
  ...
  //////////////////////////////////////////////////
  /**
   * Possible results of pre-evaluation of shot parameters for current game
   * state.
   */
  enum PreProcessCode {
    PPC_BADPARAMS = 0, /**< Shot parameters not satisfactory for game state. */
    PPC_G_NOEXECUTE, /**< Decision information in shot is sufficient. No need to
                        execute shot. */
    PPC_G_NORMAL,  /**< Shot should be processed normally. Cue ball is not to be
                      moved. */
    PPC_G_PLACECUE /**< Shot should be processed normally. Cue ball is to be
                      placed at requested position. */
  };
...
protected:
  ...
  // Returns an emum: bad params, don't execute but good, and execute, execute +
  // cue position
