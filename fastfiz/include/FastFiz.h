...
/**
 * The namespace in which all of the physics and rules components for FastFiz
 * reside.
 */
namespace Pool {
 ...
 * It can be thrown when TableState::executeShot() or
...
public:
  enum Type {
    ...
    INVALID_CUE_PARAMS, /**< The ShotParams for the Shot in the current
                           TableState were out of range or not physically
                           possible. */
   ...
   * this is.
   */
  virtual const char *what() const throw() { return "Bad Shot Exception"; }
...
/**
 * A struct of the parameters for a CUE_STRIKE.  It is passed to
 * TableState::executeShot() and can be retrieved from CueStrikeEvent.
 * @see operator<<(ostream &out, const ShotParams &rhs) Writes human-readable
 * output for ShotParams objects to the output stream.
 */
struct ShotParams {
  double a; /**< The x-coordinate of the cue stick (right/left english) on the
               CUE ball in mm. */
  double b; /**< The y-coordinate of the cue stick (top/bottom english) on the
               CUE ball in mm. */
  ...
  /**
   * Constructs a ShotParams object with all the fields initialized to the given
   * values.
   * @param _a     The x-coordinate of the cue stick (right/left english) on the
   * CUE ball in mm.
   * @param _b     The y-coordinate of the cue stick (top/bottom english) on the
   * CUE ball in mm.
   * @param _theta The elevation of the cue stick in degrees.
   * @param _phi   The azumith angle (heading) of the cue stick in degrees.
   * @param _v     The velocity of the cue stick before impact in m/s (max is
   * 4.5 m/s).
   */
...
/**
 * A two-dimensional Point class. It has methods for a few simple point
 * operations.
 * @see operator<<(ostream &out, const Point &rhs) Writes human-readable output
 * for Point objects to the output stream.
 */
struct Point {
...
#ifndef SWIG
  /**
   * Calculates the rotation of a Point object about the origin.  The angle of
   * rotation is counterclockwise.
   * @param cos_phi The cos of the angle to be rotated by.
   * @param sin_phi The sin of the anlge to be rotated by.
   * @return A new Point object that is a copy of this Point object rotated
   * about the origin by the angle
   ...
   * @return A new Vector object that is a copy of this Point object with the z
   ...
   * @return A new Point object that has the respective x and y components of
  ...
  /**
   * Returns a new Point object that has the respective x and y components of
   * the other Point object
  ...
  /**
   * Writes a machine-readable string representation of this Point object (a
   * sequence of space-separated
  ...
  /**
   * Returns a string object that is a machine-readable string representation of
   * this Point object
...
/**
 * A three-dimensional Vector class. It has methods for various vector
 * operations.
 * @see operator<<(ostream &out, const Vector &rhs) Writes human-readable output
 * for Vector objects to the output stream.
 */
struct Vector {
...
#ifndef SWIG
  /**
   * Returns a new Vector object that has the respective x, y, and z components
  ...
  /**
   * Returns a new Vector object that is a copy of this Vector object rotated
   * about the origin.
   ...
   * Returns a new Vector object that is a copy of this Vector object rotated
   * about the origin.
   * The parameters are the cos and sin of the angle to be rotated by.  The
...
public:
  ...
  static constexpr double VELOCITY_EPSILON =
      1E-10; /**< threshold velocity for balls to be considered stationary. */
  ...
  /** converts an angle in radians to be on the specified range, either [0,2*pi)
   * or [-pi,pi) */
 ...
 *  It is mostly used internally for execution of the physics, and as part of a
 * TableState object.
 */
class Ball {
public:
#ifndef SWIG
  static constexpr double MU_BALL_BALL =
      0.01; /**< coefficient of ball-ball friction */
  static constexpr double MU_CUETIP_BALL =
      0.7; /**< coefficient of cue tip-ball friction */
  static constexpr double BALL_COEFF_REST_POS =
      2.0; /**< coefficient of restitution for balls */
  static constexpr double BALL_COEFF_REST_NEG =
      0.0; /**< coefficient of restitution for balls */
  ...
  enum State {
    NOTINPLAY,  /**< not on the table */
    STATIONARY, /**< in play but not moving */
    SPINNING,   /**< not moving translationally, but spinning in place about the
                   vertical axis */
    SLIDING, /**< sliding across the surface of the table, not necessarily in a
                straight trajectory */
    ROLLING, /**< rolling across the surface of the table, necessarily in a
                straight trajectory */
  ...
  /** Create a new ball */
  Ball()
      : radius(BALL_RADIUS), r(), state(UNKNOWN_STATE), type(UNKNOWN_ID), v(),
        w() {}

  /** Create a new ball with specified number */
  Ball(Type _type)
      : radius(BALL_RADIUS), r(), state(UNKNOWN_STATE), type(_type), v(), w() {}
#ifndef SWIG
  /** Create a new ball with specified number and state */
  Ball(Type _type, State _state)
      : radius(BALL_RADIUS), r(), state(_state), type(_type), v(), w() {}
  /** Create a new ball with specified number, state, and position */
  Ball(Type _type, State _state, Point _r)
      : radius(BALL_RADIUS), r(_r), state(_state), type(_type), v(), w() {}
  /** Create a new ball with specified number, state, and position */
  Ball(Type _type, State _state, double x, double y)
      : radius(BALL_RADIUS), r(x, y), state(_state), type(_type), v(), w() {}
#endif /* ! SWIG */

  /** Copy constructor */
  Ball(const Ball &rhs)
      : radius(rhs.radius), r(rhs.r), state(rhs.state), type(rhs.type),
        v(rhs.v), w(rhs.w) {}
#ifndef SWIG
  /** Assignment operator */
  ...
  bool isInPlay() const {
    return (state == STATIONARY || state == SPINNING || state == SLIDING ||
            state == ROLLING);
  }
  /** Returns true iff the ball is in a pocket. */
  bool isPocketed() const {
    return (state == POCKETED_SW || state == POCKETED_W ||
   ...
   * No overlap is ensured if the distance is greater than the sum of the
   * radiuses of the balls.
   */
  double dist(const Ball &other) const;
  /** Add small gaussian noise to ball's position based on dither parameter
   * passed.
   * This method is used for racking and spotting noise, and has nothing to do
   * with shot execution noise.
   */
...
#ifndef SWIG
  /** Print a machine-readable representation of the ball's type, radius and
   * position.
...
/** Class representing physical properties of a pool table. This does not
 * include the position of the balls.
 * In general, this object doesn't need to be instansiated more than once.
 * This class also provides some static utility functions and constants related
 * to the pool table.
 */
class Table {
public:
  ...
  static constexpr double MU_SLIDING =
      0.2; /**< coefficient of sliding friction */
  static constexpr double MU_ROLLING =
      0.015; /**< coefficient of rolling friction */
  static constexpr double MU_SPINNING =
      0.044; /**< coefficient of spinning friction */

  /* Default table parameters */
  static constexpr double TABLE_LENGTH = 2.236; /**< table length [m] */
  static constexpr double TABLE_WIDTH = 1.116;  /**< table width [m] */
  static constexpr double CORNER_POCKET_WIDTH =
      0.11; /**< corner pocket width [m] */
  static constexpr double SIDE_POCKET_WIDTH =
      0.12;                                       /**< side pocket width [m] */
  static constexpr double RAIL_HEIGHT = 0.040005; /**< rail height [m] */
  static constexpr double CUE_LENGTH = 1.45;      /**< cue length [m] */
  static constexpr double RAIL_VEL_DAMPING_X =
      0.6; /**< damping factor for velocity component parallel to rail */
  static constexpr double RAIL_VEL_DAMPING_Y =
      0.9; /**< damping factor for velocity component perpendicular to rail */
  static constexpr double RAIL_SPIN_DAMPING =
      0.1; /**< damping factor for angular velocity component */
  static constexpr double RAIL_VEL_ANGLE_ADJ =
      0.0; /**< angle adjustment factor for velocity vector */
  static constexpr double RAIL_ZSPIN_ANGLE_ADJ =
      0.0; /**< angle adjustment factor for vertical component of angular
              velocity vector */

  // used in CueStrikeEvent::doHandle
  static constexpr double CUE_MASS = 600.0;
  static constexpr double I =
      0.4 * 160.0 * 0.028575 * 0.028575 *
      0.995473; /**< moment of inertia; to match poolfiz */
  ...
  Table()
      : _muS(MU_SLIDING), _muR(MU_ROLLING), _muSp(MU_SPINNING),
        _railHeight(RAIL_HEIGHT), _cueLength(CUE_LENGTH),
    ...
    calculateTableDimensions(TABLE_LENGTH, TABLE_WIDTH, CORNER_POCKET_WIDTH,
                             SIDE_POCKET_WIDTH);
  }

  /** Constructs a table with the given parameters and automatically sets the
   *pockets.
   *	@param length  the length of the table in metres
   *	@param width the width of the table in metres
   *	@param cornerPocketWidth the width of corner pockets (horn to horn) in
   *metres
   *	@param sidePocketWidth the width of the side pockets (horn to horn) in
   *metres
   ...
   *	@param railHeight the height of the top of the rail above the table in
   *metres
  ...
  Table(double length, double width, double cornerPocketWidth,
        double sidePocketWidth, double muS = MU_SLIDING,
        double muR = MU_ROLLING, double muSp = MU_SPINNING,
        double railHeight = RAIL_HEIGHT, double cueLength = CUE_LENGTH,
  ...
  /** Returns the right corner coordinates of the given pocket (in metres),
   *looking
   *	at the pocket from the center of the table. */
  const Point &getPocketRight(Pocket pocket) const;

  /** Returns the left corner coordinates of the given pocket (in metres),
   *looking
   *	at the pocket from the center of the table. */
  const Point &getPocketLeft(Pocket pocket) const;

  /** Contains a single default table object whose reference is returned when
   * called */
  ...
  /** Returns the pocketed ball state corresponding to the given pocket (by
   * Pocket) */
...
private:
  ...
  void calculateTableDimensions(double length, double width,
                                double cornerPocketWidth,
                                double sidePocketWidth);
#endif /* ! SWIG */
};

/** Base class of the Event heirarchy. Indicates a notable event in the
 * execution of a shot.
 *
...
public:
  ...
  enum Type {
    NO_EVENT,       /**< Nothing happened (only used internally) */
  ...
  /** Returns the physical information (location, velocity,spin) of the first
   * (possibly only) ball involved in the event.
   * May cause a null pointer exception if the information is not available (if
   * event was not yet handled).
   */
  ...
  /** Used within physics code to apply event to a table state, modifying ball
   * states, positions, and properties
   * Also updates ball information within event.
   */
  void handle(TableState &ts, bool VERBOSE = false) {
    doHandle(ts, VERBOSE);
    copyBalls(ts);
  }
...
private:
  ...
  const Event &operator=(const Event &rhs); //{if (this != &rhs)
                                            //{_time=rhs._time;
                                            //_ball1=rhs._ball1;}; return
                                            //*this;}
#endif                                      /* ! SWIG */
};

/** An event invloving a physical transition of state, e.g between SLIDING and
 * ROLLING.
 * In general these events are important for graphical simulation of the shot,
 * and for trajectory analysis.
 */
...
private:
  ...
  StateChangeEvent &
  operator=(const StateChangeEvent &
                rhs); //{if (this != &rhs) Event::operator=(rhs); return *this;}
#endif                /* ! SWIG */
};
...
public:
  BallCollisionEvent(double time, Ball::Type b1, Ball::Type b2)
...
private:
  // copy and assignment
  BallCollisionEvent(
      const BallCollisionEvent &rhs); //: Event(rhs), _ball2(rhs._ball2) {}
...
private:
  // copy and assignment
  RailCollisionEvent(
      const RailCollisionEvent &rhs); //: Event(rhs), _rail(rhs._rail) {}
...
private:
  // copy and assignment
  PocketedEvent(
      const PocketedEvent &rhs); //: Event(rhs), _pocket(rhs._pocket) {}
...
private:
  // copy and assignment
  CueStrikeEvent(
      const CueStrikeEvent &rhs); //: Event(rhs), _params(rhs._params) {}
 ...
 * The private methods in this class implement the main part of the physics
 * simulation.
 */
class Shot {
public:
...
private:
  ...
  Shot(TableState &state, const ShotParams &sp,
       bool fullSim /* false = get first ball hit only */, bool verbose = false,
       bool errors = false)
      : shotEvents() {
  ...
  void simulateShot(TableState &state, const ShotParams &sp, bool fullSim,
                    bool verbose, bool errors);

  static bool VERBOSE;
  static bool CHECK_ERRORS;
  static void updateTime(TableState &state, double oldTime, double newTime);
  static void updateBall(const Table &table, Ball &ball, double oldTime,
                         double newTime);
  ...
  static int solveQuartic(double roots[], double a0, double a1, double a2,
                          double a3, double a4);
  static constexpr double NEAR_FUTURE_EPSILON = 1E-8;
  static double leastPositiveRealRoot(double roots[],
                                      double epsilon = NEAR_FUTURE_EPSILON);
...
// static constexpr double R = 0.028575; //Ball radius //Now taken from
// Ball::BALL_RADIUS
// static constexpr int NUM_BALLS = 16; //Now taken from
// TableState::getNumBalls()
#endif /* ! SWIG */
};
#ifndef SWIG
...
public:
  /* Thresholds */
  static constexpr double MAX_VELOCITY =
      10; /**< maximum velocity allowed [m/s]  */
  static constexpr double MIN_THETA =
      0.0; /**< minimum theta allowed [degrees] */
  static constexpr double MAX_THETA =
      70.0; /**< maximum theta allowed [degrees] */

  /** This integer is used within the bitmask integer returned by
   *Table::isPhysicallyPossible()
   *	and Table::isValidBallPlacement() (as well as the fizG versions of those
   *classes). It
   *	indicates that all shot parameters are valid; the actual integer
   *returned by these methods
   *	is simply the OR'ed combinations of the all the possibilities from the
   *list under the heading
   *	'Variables' in the HTML documentation or listed below in the header
   *file; you can AND the result
   *	returned by these methods with any of the possibilities from the list
   *below to test if that
   ...
   *	and Table::isValidBallPlacement() (as well as the fizG versions of those
   *classes).  It
   *	indicates that shot parameter 'theta' is invalid or not physically
   *possible;
   *	the actual integer returned by these methods
   *	is simply the OR'ed combinations of the all the possibilities from the
   *list under the heading
   ...
   *	and Table::isValidBallPlacement() (as well as the fizG versions of those
   *classes).  It
   *	indicates that shot parameter 'phi' is invalid or not physically
   *possible;
   *	the actual integer returned by these methods
   *	is simply the OR'ed combinations of the all the possibilities from the
   *list under the heading
   ...
   *	and Table::isValidBallPlacement() (as well as the fizG versions of those
   *classes).  It
   *	indicates that the cue will collide with a ball before striking the cue
   *ball;
   *	the actual integer returned by these methods
   *	is simply the OR'ed combinations of the all the possibilities from the
   *list under the heading
   ...
   *	and Table::isValidBallPlacement() (as well as the fizG versions of those
   *classes).  It
   *	indicates that the ball placement x-y coordinates result in an overlap
   *with another ball on table;
   *	the actual integer returned by these methods
   *	is simply the OR'ed combinations of the all the possibilities from the
   *list under the heading
   *	'Variables' in the HTML documentation or listed below in the header
  ...
  /** Create an empty table state for given table (or default table if not
   * specified) */
  TableState(const Table &table = Table::defaultTable())
      : _table(table), balls() {}
  /** Copy constructor */
  ...
  /** Get ball information based on id. If ball does not exist it will be added
  ...
  /** Returns BAD_X_VAL or BAD_Y_VAL if any ball that is in play is off the
   *table or
   *	BALL_OVERLAP if any ball is overlapping with another ball.
   *	Used to check cue ball placement validity in the case of Ball In Hand,
   *for example.
   *	@param VERBOSE Print debugging information */
  int isValidBallPlacement(bool VERBOSE = false) const;

  /** Checks physical validity of the given shot parameters and table state.
   *Used before calling
   *	executeShot() to double-check that an error condition will not result
   *due to invalid
   *	parameters.
   *  @param shotParams Shot parameters
   *	@param VERBOSE Print debugging information
   */
  int isPhysicallyPossible(const ShotParams &shotParams,
  ...
  %newobject executeShot;
        ...
        * @exception BadShotException will be thrown in case of bad parameters or
        * errors in execution.
        */
  Shot *executeShot(const ShotParams &sp, bool verbose = false,
                    bool errors = false) {
   ...
   * the cue ball and then return the ball type, or UNKNOWN_ID if no ball is
   * hit.
   */
...
private:
  ...
  static constexpr double EPSILON_B = 0.001; /**< small value added to the b
                                                shot parameter for threshold
                                                calculations [mm] */
  static constexpr double EPSILON_THETA =
      0.001; /**< small value added to the theta shot parameter for threshold
                calculations [degrees] */
  static constexpr int MAX_BALLS_ON_TABLE =
      100; /**< Maximum balls allowed on the table at any time. */

  static int numLineSphereIntersections(Vector &p1, Vector &p2, Vector &p3,
